...
##### CONFIG
# Manually set the global Tenant ID for CyberCNS V4 instance, if not passed in via variable or parameter already:
...
# NinjaRMM Custom Documentation Field Name (third can be secure, all must have Script Read permissions)
...
if ($CompanyID -is [int] -and $CompanyID -ge 100 -and $CompanyID -le 9999) {
    ...
    write-host "Format should be: ### (3 or 4 integer digits)"
    exit 1
}
