<# Extend-WinRE-RecoveryPartition.ps1
SOURCE: https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/add-update-to-winre?view=windows-11#extend-the-windows-re-partition
...
The sample script below can be used to increase the size of recovery partition to enable successfully update 
Windows Recovery Environment (WinRE). Updates to WinRE require at minimum 250MB of free space in the 
recovery partition to install successfully. On devices that may not have adequate free space in the recovery 
partition, the sample script below can be used to extend the recovery partition.

Reboot your machine before you run the script. This is critical as there may be pending partition operations 
staged on your machine that will need to be finalized before the script can safely increase the WinRE 
partition size. After your machine reboots open Powershell as admin and run 

mkdir <path to new backup directory> 

to create a backup directory that the script may use in case of failure to restore the original partition. 
Note the location of this backup directory as the script will ask for your backup path.

ADDED by David Szpunar on 2024-02-10 v0.0.1:
    ...
    You can create Ninja Script Variables for SkipConfirmation (checkbos) and BackupFolder (text box) and they will work.
...
ADDED by David Szpunar on 2024-03-21 v0.0.3:
	...
	https://discord.com/channels/676451788395642880/1206063878564352081/1217215431102697593
	and
...
#>
Param (
[Parameter(Mandatory=$false,HelpMessage="Skip confirmation")][switch]$SkipConfirmation,
[Parameter(Mandatory=$false,HelpMessage="Path to backup old WinRE partition content to")][string]$BackupFolder = "C:\LenexTemp\ExtendWinREBackup"
)

##### CONFIG ####
# Define Ninja Custom Field Names, the first to receive just the final line of output (single line text field), and 
# the second to receive the full output (multi-line text field), both Automation-writable. If set to blank or $null, 
# will not output or attempt to output anything to custom fields.
$NinjaCustomField_LastResult = 'kb5034441LastResult'
$NinjaCustomField_LastRunFull = 'kb5034441LastFullRun'
##### END CONFIG ####
...
# Initialize Ninja Custom Field for new run, clear any previous log output:
if(![string]::IsNullOrWhiteSpace($NinjaCustomField_LastRunFull)) {
    Ninja-Property-Set $NinjaCustomField_LastRunFull ""
}
...
if(!(Test-Path -Path "$BackupFolder" -PathType Container)) {
    if($SkipConfirmation) {
        Write-Host "Creating backup folder $BackupFolder as it does not exist."
        New-Item -ItemType Directory -Path "$BackupFolder"
    } else {
        Write-Host "In write-mode, would create backup folder $BackupFolder as it does not exist."
    }
}
if(Test-Path -Path "$BackupFolder" -PathType Container) {
    if(!((Get-ChildItem $BackupFolder -force | Select-Object -First 1 | Measure-Object).Count -eq 0))
    {
        Write-Host "WARNING: The backup folder $BackupFolder is not empty. Quitting, only works with an empty folder!"
       exit 1
    }
}
# Log message
function LogMessage([string]$message)
{
	$message = "$message"
	Write-Host $message
    if(![string]::IsNullOrWhiteSpace($NinjaCustomField_LastResult)) {
        Ninja-Property-Set $NinjaCustomField_LastResult $message
    }
    if(![string]::IsNullOrWhiteSpace($NinjaCustomField_LastRunFull)) {
        $fullmessage = Ninja-Property-Get $NinjaCustomField_LastRunFull
        $fullmessage += "`r`n$message"
        $fullmessage | Ninja-Property-Set-Piped $NinjaCustomField_LastRunFull
    }
}
...
function DisplayPartitionInfo([string[]]$partitionPath)
{
	...
	LogMessage("  Partition capacity: " +  ($volume.Capacity / 1024 / 1024) + " MB")
	LogMessage("  Partition free space: " + ($volume.FreeSpace / 1024 / 1024) + " MB")
...
if (!$WinREStatus)
{
	LogMessage("Error: WinRE Disabled - Or not run as system")
	exit 1
}
...
LogMessage("OS partition size: " + ($OSPartition.Size / 1024 / 1024) + " MB")
...
{
	LogMessage("ERROR: WinRE and OS are on the same partition, should not perform extension")
	exit 0
}
...
{
	LogMessage("RERUN: More than 250 MB of free space was detected in the WinRE partition, there is no need to extend the partition")
	exit 0
}
...
if ($NeedCreateNew)
{
	...
	LogMessage("Will shrink OS partition by " + ($shrinkSize / 1024 / 1024) + " MB")
	LogMessage("  Current OS partition size: " + ($OSPartition.Size / 1024 / 1024) + " MB")
	LogMessage("  Target OS partition size after shrinking: " + ($targetOSPartitionSize / 1024 / 1024) + " MB")
	LogMessage("New WinRE partition will be created with size: ", ($targetWinREPartitionSize / 1024 / 1024) + " MB")
	if ($WinREIsOnSystemPartition)
	{
	...
	else
	{
		...
		LogMessage("  Current WinRE partition size: " + ($WinREPartitionSizeInfo[0] / 1024 / 1024) + " MB")
	}
}
else
{
	...
	LogMessage("  WinRE partition: Disk [" + $OSDiskIndex + "] Partition [" + $WinREPartitionIndex + "]")
	LogMessage("  Current WinRE partition size: " + ($WinREPartitionSizeInfo[0] / 1024 / 1024) + " MB")
	LogMessage("  New WinRE partition size:     " + ($targetWinREPartitionSize / 1024 / 1024) + " MB")
...
else
{
	...
	# 	exit 0
	# }
    LogMessage("READ MODE: To write changes you must rerun with the SkipConfirmation parameter set to TRUE to proceed with changes. Quitting.")
    exit 0
}
LogMessage("")
