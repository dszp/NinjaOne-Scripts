...
THIS VERSION CONVERTS PAUSES TO PARAMETERS BUT DOES NOT HAVE ANY NINJARMM CUSTOM FIELDS OR OTHER MODIFICATIONS
#>
Param (
[Parameter(Mandatory=$false,HelpMessage="Skip confirmation")][switch]$SkipConfirmation,
[Parameter(Mandatory=$false,HelpMessage="Path to backup old WinRE partition content to")][string]$BackupFolder = "$env:temp\ExtendWinREBackup"
)
# ------------------------------------
# Helper functions
# ------------------------------------
...
else
{
	...
	# 	exit 0
	# }
    LogMessage("REPORT-ONLY mode, must rerun with -SkipConfirmation flag to proceed with changes. Quitting.")
    exit 0
}
LogMessage("")
